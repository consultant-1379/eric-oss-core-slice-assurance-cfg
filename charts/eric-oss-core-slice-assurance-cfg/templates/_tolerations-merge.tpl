...
*/}}
{{- define "eric-oss-core-slice-assurance-cfg.merge-tolerations" -}}
  {{- if (.root.Values.global).tolerations }}
      ...
      {{- range $globalItem := $globalTolerations -}}
        {{- $globalItemId := include "eric-oss-core-slice-assurance-cfg.merge-tolerations.get-identifier" $globalItem -}}
        {{- range $serviceItem := $serviceTolerations -}}
          {{- $serviceItemId := include "eric-oss-core-slice-assurance-cfg.merge-tolerations.get-identifier" $serviceItem -}}
      ...
      {{- range $globalItem := $globalTolerations -}}
        ...
        {{- range $matchItem := $matchingItems -}}
          {{- $matchItemId := include "eric-oss-core-slice-assurance-cfg.merge-tolerations.get-identifier" $matchItem -}}
...
*/}}
{{ define "eric-oss-core-slice-assurance-cfg.merge-tolerations.get-identifier" }}
  {{- $keyValues := list -}}
